...
krill_fishery_landings <- read_csv("CCAMLR_Statistical_Bulletin_Volume_35/catch_and_effort/combined.csv") %>% 
...
krill_fishing_by_nation_yr_plot <- ggplot(krill_fishing_by_nation_yr_comb,
  ...
  scale_fill_manual(values = pal_ct) +
  theme_classic(base_size = 16) +
  scale_x_continuous(limits = c(1970, 2023), breaks = seq(1970, 2020, by = 10))
...
whaling_kf_by_nation_yr <- ggplot() +
  ...
  scale_y_continuous(
    name = "Whale catch (individuals)", 
    limits = c(0, 40000),
    labels = scales::comma,
    sec.axis = sec_axis(
      ~ . * 15, name = "Krill catch (tonnes)",
